interface SuggestionButtonProps {
    ...
    onSubmitSuggestion: (fullText: string) => void; // New prop for submitting the suggestion
  ...
  const SuggestionButton: React.FC<SuggestionButtonProps> = ({ primaryText, secondaryText, onSubmitSuggestion }) => {
    ...
    return (
      ...
      <button
        onClick={() => onSubmitSuggestion(fullText)} // Make sure this matches the prop name
  ...
  const SuggestionButtons: React.FC<SuggestionButtonsProps> = ({ onSubmitSuggestion }) => {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 justify-center"> {/* Use 1 column on mobile and 2 columns on larger screens */}
